...
//  int ieta_compare2_min=1, ieta_compare2_max=7, iphi_compare2_min=1, iphi_compare2_max=72, idepth_compare2_min=1, idepth_compare2_max=1; // channels to be checked, inclusive
  ...
  TH1F *HBHE_RecHitTiming[nieta][_length][ndepth][nmethod];
  TH1F *HBHE_RecHitEnergy[nieta][_length][ndepth][nmethod];
  ...
  TH1F *HBHE_RecHitEnergy_off[nieta][ndepth][nmethod];
  TH1F *HBHE_RecHitEnergy_in[nieta][ndepth][nmethod];
  TH1F *HBHE_ADC_off[nieta][ndepth][nmethod];
  TH1F *HBHE_ADC_in[nieta][ndepth][nmethod];
...
//    HBHE_NormalMap_nEvt_NEF[method] = makeTH2F("HBHE_NormalMap_nEvt_NEF_"+method_c,"HBHE Normal Events Distribution_NEF_"+method_c,60, -29.5, 30.5, 74, 0, 74);
  ...
  for(int ieta=0;ieta<nieta;ieta++){
    for(int depth=0;depth<ndepth;depth++){
      for(int method=1;method<nmethod;method++){
        TString method_c=methodNo[method];
        TString name = Form("_ieta%i_idepth%i_",ieta-minieta,depth+minidepth);
        
        HBHE_ADC_off[ieta][depth][method] = makeTH1F("HBHE_ADC_off"+name+method_c,"ADC distribution off timing"+name+method_c,1000,0,1000);
        HBHE_ADC_off[ieta][depth][method]->Sumw2();
        HBHE_ADC_in[ieta][depth][method] = makeTH1F("HBHE_ADC_in"+name+method_c,"ADC distribution in timing"+name+method_c,1000,0,1000);
        HBHE_ADC_in[ieta][depth][method]->Sumw2();
      
        HBHE_RecHitEnergy_off[ieta][depth][method] = makeTH1F("HBHE_RecHitEnergy_off"+name+method_c,energyName[method]+" distribution off timing"+name,3000,0,3000);
        HBHE_RecHitEnergy_off[ieta][depth][method]->Sumw2();
        HBHE_RecHitEnergy_in[ieta][depth][method] = makeTH1F("HBHE_RecHitEnergy_in"+name+method_c,energyName[method]+" distribution in timing"+name,3000,0,3000);
        HBHE_RecHitEnergy_in[ieta][depth][method]->Sumw2();
        
        HBHE_FC_off[ieta][depth][method] = makeTH1F("HBHE_FC_off"+name+method_c,"FC distribution off timing"+name+method_c,3000,0,3000);
    ...
    for(int phi=0;phi<_length;phi++){
      for(int depth=0;depth<ndepth;depth++){
        ...
        for(int method=1;method<nmethod;method++){          
          ...
          HBHE_RecHitTiming[eta][phi][depth][method] = makeTH1F("HBHE_RecHitTiming"+name+method_c,"HBHE Distribution of "+energyName[method]+name,YnBin[method],YminBin[method],YmaxBin[method]);
          HBHE_RecHitTiming[eta][phi][depth][method]->Sumw2();

          HBHE_RecHitEnergy[eta][phi][depth][method] = makeTH1F("HBHE_RecHitEnergy"+name+method_c,"HBHE Distribution of "+methodName[method]+name,3000,0,3000);
          HBHE_RecHitEnergy[eta][phi][depth][method]->Sumw2();
...
//    HBHEALLFC = TS0 + TS1 + TS2 + TS3 + TS4 + TS5 + TS6 + TS7;
    ...
    HBHEADC =  hbheDigi.adc(2) + hbheDigi.adc(3) + hbheDigi.adc(4);
...
//      std::cout<<"Method Num" << method << " energy and time are: " << Energy << " " << Time << std::endl;
      ...
      for(int phi=1;phi<_length;phi++){
        if(_iphi[phi]==hbhe_iphi){
          HBHE_RecHitTiming[hbhe_ieta+minieta][phi][hbhe_depth-minidepth][method]->Fill(Time);
          HBHE_RecHitEnergy[hbhe_ieta+minieta][phi][hbhe_depth-minidepth][method]->Fill(Energy);

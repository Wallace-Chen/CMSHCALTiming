...
void analysisClass::loop(){
  ...
  uuple_tree -> fChain -> SetBranchStatus("ls"              , kTRUE);
...
//  int runStop  = 318000;
  ...
  TH2F *HBHE_chargeAveragedTiming_vs_run_IEta[16];
  TH2F *HBHE_RecTiming_vs_run_IEta[16];

  TH2F *HBHE_totalCharge_vs_run_ieta11 = makeTH2F("HBHE_totalCharge_vs_run_ieta11","HB total Charge vs Run",runStop-runStart+1,runStart-0.5,runStop+0.5,100,0,2000); 
  TH2F *HBHE_chargeRatio_vs_run_ieta3_iphi66 = makeTH2F("HBHE_chargeRatio_vs_run_ieta66_ihpi3","HB total Charge vs Run",runStop-runStart+1,runStart-0.5,runStop+0.5,10,0,10); 
  TH2F *HBHE_chargeRatio_vs_run_ieta11_iphi66 = makeTH2F("HBHE_chargeRatio_vs_run_ieta11_ihpi66","HB total Charge vs Run",runStop-runStart+1,runStart-0.5,runStop+0.5,10,0,10); 
  TH2F *HBHE_chargeRatio_vs_run_ieta-13_iphi7 = makeTH2F("HBHE_chargeRatio_vs_run_ieta-13_ihpi7","HB total Charge vs Run",runStop-runStart+1,runStart-0.5,runStop+0.5,10,0,10); 

  for(int eta=0;eta<16;eta++){
    TString name = Form("abs(IEta) %i",eta+1);
  ...
  for (int i = 0; i < n_events; ++i){
    ...
    if ( (i + 1) % 1 == 0 ) std::cout << "Processing event " << i + 1 << "/" << n_events << std::endl;
...
//    if( !bxValid ) {eventSkip++;continue;} //skip if ls is not in the range
    ...
    std::cout << "hbheDigis Constrcuted\n";
...
//        HBHE_RecTiming_vs_run           ->Fill(hbhe_run,hbhe_recTime);
        ...
        if(hbhe_ieta == 11){
          HBHE_totalCharge_vs_run_ieta11->Fill(hbhe_run,HBHEALLFC);
        }
        if(hbhe_ieta == 3 && hbhe_iphi == 66){
          HBHE_chargeRatio_vs_run_ieta3_iphi66->Fill(hbhe_run,TS[4]/TS[3]);
        }
        if(hbhe_ieta == 11 && hbhe_iphi == 66){
          HBHE_chargeRatio_vs_run_ieta11_iphi66->Fill(hbhe_run,TS[4]/TS[3]);
        }
        if(hbhe_ieta == -13 && hbhe_iphi == 7){
          HBHE_chargeRatio_vs_run_ieta-13_iphi7->Fill(hbhe_run,TS[4]/TS[3]);
        }
//        if(hbhe_ieta != 0){
//          HBHE_chargeAveragedTiming_vs_run_IEta[abs(hbhe_ieta)-1]->Fill(hbhe_run,HBHETiming);

...
// 1. Charge-Averaged Timing  2. DigiRec Timing 3. RecHit Timing
  TH2F *HBHE_Timing_vs_Energy_ALL[3];
  TH2F *HBHE_Timing_vs_Energy[nieta][niphi][ndepth][3];
  TH2F *HBHE_Timing_vs_Energy_check[3];
  TH2F *HBHE_Timing_vs_Energy_compare1[3];
  TH2F *HBHE_Timing_vs_Energy_compare2[3];
// Booking histograms for Timing vs iEta
  TH2F *HBHE_Timing_vs_iEta_ALL[3];
...
//      for(int depth=0;depth<ndepth;depth++){
//        TString name = Form("_ieta%i_iphi%i_idepth%i_",eta-minieta,phi+miniphi,depth+minidepth);
//        for(int method=0;method<nmethod;method++){          
//          TString method_c=methodNo[method];
//        
...
//  TH1F *HBHE_Energy_D1		      = makeTH1F("HBHE_Energy_D1" ,"HBHE: E>5GeV Depth 1", 50, 0, 0);
  ...
  TH2F *HBHE_EnerWeiTiming            = makeTH2F("HBHE_EnerWeiTiming","HBHE: TS3>30fC, Energy Weighted Timing",60, -29.5, 30.5, 74, 0, 74);
  ...
  TH2F *HBHE_EnerWeiTiming_D1            = makeTH2F("HBHE_EnerWeiTiming_D1","HBHE: TS3>30fC, Energy Weighted Timing Depth 1",60, -29.5, 30.5, 74, 0, 74);
  ...
  TH2F *HBHE_EnerWeiTiming_D2            = makeTH2F("HBHE_EnerWeiTiming_D2","HBHE: TS3>30fC, Energy Weighted Timing Depth 2",60, -29.5, 30.5, 74, 0, 74);
...
//General END, next for every depth
	 if(hbhe_depth==1){
	 	if(TS[3]>30){
            ...
            HBHE_EnerWeiTiming_D1_sum[hbhe_ieta+30][hbhe_iphi]+= (TS[2]*50+TS[3]*75+TS[4]*100)/(TS[2]+TS[3]+TS[4]);
		}
		if(TS[3]>30 && TS[3]<100)  {
	 ...
	 if(hbhe_depth==2){ 
      ...
      HBHE_EnerWeiTiming_D2_sum[hbhe_ieta+30][hbhe_iphi]+= (TS[2]*50+TS[3]*75+TS[4]*100)/(TS[2]+TS[3]+TS[4]);
		}
		if(TS[3]>100 && TS[3]<200)  {
  ...
  for (int i=0; i<60; i++) {
      ...
      hbhe_D1_timing = HBHE_EnerWeiTiming_D1_sum[i][j] / HBHE_TSRatioMap_D1_nEvt[i][j];
      ...
      HBHE_EnerWeiTiming_D1->SetBinContent(i,j,hbhe_D1_timing);
		}
	}
   }
  ...
  std::cout<<"List of HBHE D2 channels:"<<std::endl; 
   for (int i=0; i<60; i++) {
      ...
      hbhe_D2_timing = HBHE_EnerWeiTiming_D2_sum[i][j] / HBHE_TSRatioMap_D2_nEvt[i][j];
      ...
      HBHE_EnerWeiTiming_D2->SetBinContent(i,j,hbhe_D2_timing);
		}
	}
   }
